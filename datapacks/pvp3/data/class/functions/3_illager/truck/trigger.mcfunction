...
# 召唤
execute at @e[tag=new_minion_pos] run summon minecraft:ravager ~ ~ ~ {PersistenceRequired:true,Health:32f,CustomNameVisible:true,Tags:["new_minion","fall_immune"]}
...
effect give @e[tag=new_minion,limit=1] weakness 1000000 1 true
...
execute if entity @s[team=lobby] run team join lobby @e[tag=new_minion]
execute if entity @s[team=red] run team join red @e[tag=new_minion]
execute if entity @s[team=blue] run team join blue @e[tag=new_minion]
execute if entity @s[team=green] run team join green @e[tag=new_minion]
execute if entity @s[team=yellow] run team join yellow @e[tag=new_minion]
...
execute if score @s state matches ..2 if score $mutation mem matches 3 store result entity @e[tag=new_minion,limit=1] Motion[0] double 0.035 run scoreboard players operation #x2 mem -= #x1 mem
execute if score @s state matches ..2 if score $mutation mem matches 3 store result entity @e[tag=new_minion,limit=1] Motion[1] double 0.035 run scoreboard players operation #y2 mem -= #y1 mem
execute if score @s state matches ..2 if score $mutation mem matches 3 store result entity @e[tag=new_minion,limit=1] Motion[2] double 0.035 run scoreboard players operation #z2 mem -= #z1 mem
execute unless score @s state matches ..2 store result entity @e[tag=new_minion,limit=1] Motion[0] double 0.02 run scoreboard players operation #x2 mem -= #x1 mem
execute unless score @s state matches ..2 store result entity @e[tag=new_minion,limit=1] Motion[1] double 0.02 run scoreboard players operation #y2 mem -= #y1 mem
execute unless score @s state matches ..2 store result entity @e[tag=new_minion,limit=1] Motion[2] double 0.02 run scoreboard players operation #z2 mem -= #z1 mem
