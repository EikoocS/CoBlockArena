...
execute if score @s[tag=4_in_water,scores={4_water_last=..0}] 4_water_storage matches 1.. run title @s actionbar {"color":"gold","text":"⚠ 已触发下一次水生技能","bold":true}
execute if score @s[tag=4_in_water,scores={4_water_last=..0}] 4_water_storage matches 0 run title @s actionbar [{"color":"red","text":"✖ "},{"text":"水生技能已完全耗尽","bold":true}]
